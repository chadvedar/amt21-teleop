...
import numpy as np
import time
from ExoKinematic7DoF import ExoKinematic
...
def arm_position_callback(msg:Float32MultiArray):
    ...
    current_joint_angles[0] = deg_to_rad(msg.data[0]  + 0.0)
    current_joint_angles[1] = deg_to_rad(msg.data[1] - 135.0)

    current_joint_angles[2] = deg_to_rad(-msg.data[3]*0.4 - 90.0)
    if abs(current_joint_angles[2] - prev_joint_angles[2]) > 0.5 and not prev_joint_angles[2] == 0.0:
        current_joint_angles[2] = prev_joint_angles[2]

    current_joint_angles[3] = deg_to_rad(msg.data[2])
    current_joint_angles[4] = deg_to_rad(0.0)
    current_joint_angles[5] = deg_to_rad(0.0)
